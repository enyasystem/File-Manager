"""Thin shim exporting the authoritative organizer implementation.

This module delegates to ``file_manager.organizer_impl`` which contains the
actual implementations. Keeping a tiny shim here avoids duplicated or
stale definitions being imported by the GUI or other callers.
"""

from .organizer_impl import organize_by_type, organize_by_date, undo_moves

__all__ = ["organize_by_type", "organize_by_date", "undo_moves"]
"""Thin shim exporting the authoritative organizer implementation.
...
__all__ = ["organize_by_type", "organize_by_date", "undo_moves"]
"""Thin shim exporting the clean organizer implementation.

This module delegates to `file_manager.organizer_impl` which contains the
authoritative implementations. Importing this module yields the expected
API (including the `extensions` parameter for `organize_by_type`).
"""

from .organizer_impl import organize_by_type, organize_by_date, undo_moves  # noqa: E402
...
To avoid duplicated/garbled content, this module delegates to
`file_manager.organizer_impl` which holds the authoritative functions.
"""
...
Single, clean implementation to avoid duplicate definitions.
"""
...
This file provides a single authoritative implementation (no duplicates):

- `organize_by_type(files, target_root, dry_run=True, mode='move', extensions=None)`
- `organize_by_date(files, target_root, dry_run=True, mode='move', selected_month=None, selected_year=None,
                    size_threshold_mb=None, size_folder_name='Large', date_source='ctime')`
- `undo_moves(log_path, dry_run=False)`

All functions return lists of action dictionaries. When not in dry-run mode
each run writes a JSON log named `fm_organize_<stamp>.json` into `target_root`.
"""
...
def organize_by_type(
    ...
    `extensions` is an optional list (e.g., ['png','jpg']). If provided,
    only matching extensions are processed; non-matching files are recorded
    as skipped in the action log.
    """
    actions: List[Dict] = []
    target_root.mkdir(parents=True, exist_ok=True)
...
def organize_by_type(
    ...
    `extensions` is an optional list (e.g., ['png','jpg']). If provided,
    only matching extensions are processed; others are recorded as skipped.
    """
    actions: List[Dict] = []
    target_root.mkdir(parents=True, exist_ok=True)
    ...
    for f in files:
        src = Path(f.get("path") or f.get("src") or "")
...
def organize_by_date(
    ...
    date_source: one of 'ctime', 'mtime', 'exif'. EXIF requires Pillow; if
    unavailable EXIF falls back to filesystem timestamps.
    """
    actions: List[Dict] = []
    target_root.mkdir(parents=True, exist_ok=True)
